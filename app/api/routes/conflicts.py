...
class GenerateConflictsRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "count": 50,
                "conflict_types": ["platform_conflict", "track_conflict"],
                "severity_distribution": {"high": 0.3, "medium": 0.5, "low": 0.2},
                "stations": ["London Waterloo", "Manchester Piccadilly"],
...
async def generate_conflicts(
    ...
    """
    Generate synthetic conflicts for testing and training.
    
    This endpoint uses the conflict generator to create realistic
    synthetic conflicts. Optionally embeds them and stores in Qdrant
    ...
    **Explanation:**
    The generator creates conflicts with realistic patterns:
    - Station-appropriate conflict types
    - Time-of-day correlations
    - Severity distributions matching real-world data
    - Resolution outcomes based on configurable success rates
    ...
    try:
        ...
        if request.conflict_types:
            ...
            conflicts = generator.generate(count=request.count)
        
        # Conflicts are already GeneratedConflict models
        generated = list(conflicts)
        ...
        if request.include_embeddings:
            try:
                ...
                for conflict in generated:
                    ...
                    # Store in Qdrant
                    payload = _conflict_to_payload(conflict)
                    await qdrant_service.upsert_conflict(
...
async def analyze_conflict(
    ...
    **Next Steps:**
    ...
    try:
        ...
        # Generate embedding
        embedding = embedding_service.embed_text(conflict_text)
        embedding_generated = True
