...
class QdrantService:
    ...
    def _connect(self) -> None:
        ...
        try:
            ...
            raise QdrantConnectionError(
                f"Failed to connect to Qdrant Cloud at {self.url}",
                {"error": str(e)}
            )
    ...
    def upsert_conflict(
        ...
        try:
            ...
            # Use provided ID or conflict's ID
            point_id = conflict_id or conflict.id
            
            # Build payload from conflict model
    ...
    def upsert_pre_conflict_state(
        ...
        try:
            ...
            logger.debug(f"Upserted pre-conflict state {state.id}")
